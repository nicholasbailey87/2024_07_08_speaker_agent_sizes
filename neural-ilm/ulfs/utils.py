...
def save_image_grid(
        filepath, image_grid, margin_size=0, margin_value=0, text=None, texts=None, text_size=24, text_color=0, print_filepath=False,
        resize=None
    ):
    """
    ...
    flattened_image = torch.zeros(3, gridH * H + margin_size * (gridH - 1), gridW * W + margin_size * (gridW - 1), dtype=torch.uint8)
    if margin_value != 0:
