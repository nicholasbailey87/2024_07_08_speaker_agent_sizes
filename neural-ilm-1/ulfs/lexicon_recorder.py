...
class LexiconRecorder(object):
    ...
    def print_lexicon(self):
        ...
        for action, count_by_utterance in enumerate(self.count_by_utterance_by_action):
            # print('action', action)
            l = [{'utt': utt, 'count': count} for utt, count in count_by_utterance.items()]
            ...
            utts_str = ' '.join([str(x['utt']).replace('0', '') + '=' + str(x['count']) for x in l[:5]])
            print('l[:5]', utts_str)
    ...
    def calc_stats(self):
        ...
        return stats
        # print('median unique utt', np.median(unique_by_a))
        # print('mean unique utt', np.mean(unique_by_a))
        # print('min', np.min(unique_by_a), 'max', np.max(unique_by_a))
        # print('entropy', scipy.stats.entropy(unique_by_a))
        # print('total utt entropy', scipy.stats.entropy(num_utterances_by_a))
        # print('total', total_utt, total_unique)
