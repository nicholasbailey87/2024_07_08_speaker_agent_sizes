...
class RunnerBase(object):
    ...
    def _add_standard_arguments(self, parser):
        ...
        # self._maybe_add_argument(parser, '--model-file', type=str, default='tmp/{name}_{ref}_{hostname}_{date}_{time}.dat')
        # self._maybe_add_argument(parser, '--logfile', type=str, default='logs/log_{name}_{ref}_{hostname}_{date}_{time}.log')
    ...
    def _extract_standard_args(self, args):
        ...
        self.model_file = args.model_file.format(
            **args.__dict__, hostname=name_utils.hostname(), date=name_utils.date_string(), time=name_utils.time_string())
    ...
    def maybe_load_params(self):
        ...
        if self.logfile is not None and self.logfile != '':
            print('file', self._get_caller_filename())
            self.logger = Logger(self.logfile, self.params, file=self._get_caller_filename())
        return self.params
    ...
    def log(self, logdict):
        if self.logfile is not None and self.logfile != '':
            self.logger.log(logdict)

    def maybe_print_and_log(self, stats_dict, formatstr):
        if self.should_render():
            self.print_and_log(stats_dict, formatstr)

    def print_and_log(self, stats_dict, formatstr):
