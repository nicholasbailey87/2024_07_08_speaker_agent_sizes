...
import json, os, math, time, datetime, subprocess
from os import path
...
def get_log_results(logfile, step_key, skip_record_types, value_key, record_type=None, max_step=None):
    with open(logfile, 'r') as f:
        c = f.read()
    lines = c.split('\n')
    meta = json.loads(lines[0].replace('meta: ', '').strip())
    steps = []
    values = []
        ...
        if value_key is None:
            for k in value_keys:
                if k in d:
                    ...
                    break
            if value_key is None:
                raise Exception('no keys from ' + str(value_keys) + ' found in keys ' + str(d.keys()) + ' file ' + logfile + ' label ' + label +  ' title ' + title)
...
def plot_logfile2(
        logfile, step_key, skip_record_types, value_key, units='thousands', record_type=None, y_lims=None, title=None, label=None,
        x_lims=None, max_step=None
    ):
...
def plot_groups2(value_key='eval_acc'):
    for group_def in groups:
